...
def main():
	...
	parser.add_argument('installers', nargs='+', help='Paths to installer files to upload.')
	options = parser.parse_args()

	private_key = rsa.PrivateKey.load_pkcs1(binascii.a2b_base64(options.private_key))

	for archive in options.installers:
		spec = archive.split('-')[-1]
		if re.match(r'^macos_\d+_\d+_universal2.whl$', archive):
			platform = 'u'
		elif re.match(r'^manylinux_\d+_\d+_aarch64.whl$', archive):
			platform = 'a'
		elif re.match(r'^manylinux_\d+_\d+_x86_64.whl$', archive):
			platform = 'l'
		elif re.match(r'^win_amd64.whl$', archive):
			platform = 'w'
		else:
			platform = ''
